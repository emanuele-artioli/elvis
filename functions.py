...
# def filter_squares(squares: np.array, stride: int, blocks_to_keep: tuple) -> Union[np.array, np.array]:
#     """
#     Filter squares based on their indices:
...
#     """
#     n, m, _, _, _ = squares.shape
#     # every block in the mask is white, inpaint everything
#     filter_mask = np.ones(squares.shape, dtype=int) * 255
#     # except the blocks selected in blocks_to_keep, therefore set them black
#     for (i, j) in blocks_to_keep:
#         filter_mask[i::stride, j::stride] = 0

#     # now remove from the frame squares the ones that are black in the mask
#     filtered_squares = squares[::stride, ::stride]
